...
Faturamento = namedtuple('Faturamento', ['id', 'data', 'dia_da_semana', 'faturamento', 'mes'])

#--------------------------------------Resgatando os dados

with open('faturamentoAgosto.json', 'r', encoding='utf-8') as arquivo:
...
for item in faturamentos:
    if item.dia_da_semana != 'sábado' and item.dia_da_semana != 'domingo' and item.dia_da_semana != 'feriado':
        soma += item.faturamento        
        listaFaturamento.append(item.faturamento)
